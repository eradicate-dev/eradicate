...
Since this is a simulated dataset, we know exactly what the population size was before the eradication program began (676 rabbits).  Before we start we load various the packages necessary for our analysis ...
...
Which indicates that the estimate for the initial number of rabbits for model 1 was `r ests1$Nhat$Nhat[1]` and for model 2 was `r ests3$Nhat$Nhat[1]`. The actual number of rabbits on the island was 676 so estimates from both models are reasonable. We can also get an estimate of the residual number of rabbits following the final removal period
```{r}
