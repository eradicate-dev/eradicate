...
... and then read in the shapefile of the region and a raster of habitat values. 
...
In order to get estimates of either the proportion of sites occupied or total abundance, we can use the function `calcN()`, which also requires an additional argument of covariate values for each site used for prediction.  Since we wish to predict for the sites at hand, we simply use `site.data`.
```{r}
calcN(m1, site.data)
calcN(m2, site.data)
calcN(m3, site.data)
...
The return values for `calcN` contain the predicted occupancy or abundance for each site as well as for the  total population over all sites. It also produces two estimates of confidence intervals, the standard 
normal approximation and additional intervals using a log-normal transformation.  The latter intervals are guaranteed not to include zero in the interval.
...
Using data on extra detections has resulted in a higher estimate of the initial population size compared with the model without extra detections.  This is due to the fact that cats were detected in a camera in the periods following the last capture of a cat in the leg-hold traps.  We can get an estimate of the residual population as follows ... [TODO: function to give proper estimate with uncertainty]
...
... which indicates 12 cats still extant using inference from the removal + extra detections model compared with nominal eradication using the removal only model.
