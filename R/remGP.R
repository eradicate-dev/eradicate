...
#' secondary periods.
#'
#' @usage remGP(lamformula, phiformula, detformula, data, mixture = c("P", "NB"), K,
...
#' @export
#'
remGP <- function(lamformula, phiformula, detformula, data,
                     mixture=c('P', 'NB'), K, starts, method = "BFGS", se = TRUE, ...)
{
  if(!is(data, "eFrameRGP"))
    stop("Data is not a eFrameRGP.")
